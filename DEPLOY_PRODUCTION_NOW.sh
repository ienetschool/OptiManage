#!/bin/bash
# IMMEDIATE PRODUCTION DEPLOYMENT - UPDATE ENDPOINTS

echo "🚀 DEPLOYING UPDATE ENDPOINTS TO PRODUCTION..."

# Connect to production server and deploy
sshpass -p '&8KXC4D+Ojfhuu0LSMhE' ssh -o StrictHostKeyChecking=no root@5.181.218.15 << 'REMOTE_COMMANDS'
...
# Create updated medicalRoutes.ts with ALL UPDATE endpoints
...
export function registerMedicalRoutes(app: Express) {
  ...
  // Doctor UPDATE endpoint - CRITICAL
...
echo "✅ medicalRoutes.ts updated with ALL UPDATE endpoints"
...
curl -s -X PUT http://localhost:8080/api/patients/test -H "Content-Type: application/json" -d '{"firstName":"TestUpdate"}' | head -c 100

echo "✅ PRODUCTION UPDATE ENDPOINTS DEPLOYED!"
